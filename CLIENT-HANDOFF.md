...
3. **Convert visitors** — every page funnels to the inquiry form or a phone call. Leads land in Coraline automatically.
...
- **inquiry form**: lives on the contact page only, loaded lazily so it never slows the page down. Every other page ends in a call-to-action band that links straight to it (`/contact/#form`) — one clear conversion path instead of a heavy form repeated on every page.
...
3. **Submit a test lead** through the inquiry form on the live site and confirm it appears in Coraline.
